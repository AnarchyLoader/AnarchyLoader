    ...
    fn fetch_github_users(&mut self, endpoint: &str, user_type: &str) {
        ...
        match client.get(&api_url).call() {
            Ok(response) => {
                ...
                let users_data: Vec<serde_json::Value> = match serde_json::from_str(&body) {
                    Ok(v) => v,
                    Err(e) => {
                        log::error!(
                            "<ABOUT_TAB> Failed to parse {} JSON: {}",
                            user_type_clone,
                            e
                        );
                        match &user_type_clone[..] {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                ui.vertical_centered(|ui| {
                    ...
                    if self.app.stats.opened_count == 1 {
                        ...
                        ui.label(format!("Opened {} times", self.app.stats.opened_count));
                    }

                    let mut sorted_inject_counts: Vec<(&String, &u64)> =
                        self.app.stats.inject_counts.iter().collect();
                    ...
                    if !self.app.stats.has_injections() {
                        ...
                        sorted_inject_counts
                            .iter()
                            .take(3)
                            .for_each(|(hack_dll, count)| {
                ...
                ui.vertical_centered(|ui| {
                    ui.horizontal_wrapped(|ui| {
                        ...
                        let width = body_font.size * 0.6; // approximate space glyph width
